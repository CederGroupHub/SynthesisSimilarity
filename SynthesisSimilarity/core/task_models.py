...
class MaterialDecoder(keras.Model):
    def __init__(self,
        ...
        if self.final_activation in {'gelu', }:
        ...
        if self.target_ele_counts is not None:
            ...
            self.element_frequency = tf.constant(
                np.array(self.target_ele_counts).astype(np.float32)/self.num_train_reactions
            )
            self.balance_coeff = self.element_frequency/(1-self.element_frequency)
        else:
            self.target_ele_mask = np.ones((len(self.target_ele_labels),)).astype(np.bool)
    ...
    def _loss_mse(self, materials, materials_featurized):
        ...
        # loss_mat: (None, )
        loss_mat = tf.reduce_sum(tf.math.squared_difference(
    ...
    def _loss_mse_crossing(self, materials, materials_featurized):
        ...
        # loss_mat: (None, )
        loss_mat = tf.pow(tf.reduce_sum(loss_mat, -1), 2) - \
                   tf.reduce_sum(tf.pow(loss_mat, 2), -1)
    ...
    def _loss_error_rate(self, materials, materials_featurized):
        ...
        # loss_mat: (None, )
        y_error = tf.pow(tf.reduce_sum(y_error, -1), 2) - \
                   tf.reduce_sum(tf.pow(y_error, 2), -1)
        y_error = y_error/2.0
        # loss_mat: (None, )
        loss_mat = y_error
        mask = tf.cast(mask, tf.float32)
    ...
    def _loss_cross_entropy(self, materials, materials_featurized):
        ...
        if self.balance_PN:
            # x_mat: (None, num_eles)
            x_mat = self.__call__(x_mat, mask=mask, return_probability=True)
            loss_mat = y_real*(-tf.math.log(x_mat)) + \
                       (1-y_real)*(-tf.math.log(1-x_mat))*self.balance_coeff
        else:
            # x_mat: (None, num_eles)
            x_mat = self.__call__(x_mat, mask=mask, return_probability=True)
            loss_mat = y_real*(-tf.math.log(x_mat)) + \
                       (1-y_real)*(-tf.math.log(1-x_mat))
        # loss_mat: (None, )
    ...
    def predict_elements(self, targets, output_thresh=0.5):
        ...
        for a_y in y_pred:
            ele_list_pred = self.target_ele_labels[a_y>output_thresh].numpy().astype('U')
...
class MaterialVariance(keras.Model):
    def __init__(self,
                 mat_feature_len,
                 latent_dim,
                 mat_encoder,
        ...
        self.mat_encoder = mat_encoder
        self._all_loss = {
            'dot_sim': self._loss_dot_sim,
            'abs_dot_sim': self._loss_abs_dot_sim,
            'norm_mean': self._loss_norm_mean,
        }
        self.loss_fn_name = loss_fn
        self._loss_fn = self._all_loss[self.loss_fn_name]
    ...
    def call(self, inputs):
        ...
        :return: ((None, latent_dim)
        """

        # x_mat (None, latent_dim)
        x_mat_mean, x_mat_log_var, x_mat = self.mat_encoder(inputs)

        return x_mat


    def _loss_dot_sim(self, materials_featurized):
        """
        sum_i sum_{j \neq i} (x_i^T x_j)
        differenciate mat from a random sampled material to increase the variance
        :param mat_pairs_featurized: (None, mat_feature_len)
        :return: (None, ) for each batch data
        """

        # x_mat (None, latent_dim)
        x_mat = self.__call__(materials_featurized)

        # Attention: assume encoding is normalized already
        # TODO: to normalize when VAE is used later

        # xi_xj (None, None)
        xi_xj = tf.matmul(
            x_mat, x_mat, transpose_a=False, transpose_b=True
        )
        # xi_xj (None, )
        xi_xj = tf.reduce_sum(xi_xj, -1)
        # xi_xj (None, )
        xi_xi = tf.reduce_sum(tf.pow(x_mat, 2), -1)
        # loss_mat_variance (None, )
        loss_mat_variance = xi_xj - xi_xi

        return loss_mat_variance

    def _loss_abs_dot_sim(self, materials_featurized):
        """
        sum_i sum_{j \neq i} (|x_i^T x_j|)
        differenciate mat from a random sampled material to increase the variance
        :param mat_pairs_featurized: (None, mat_feature_len)
        ...
        # xi_xj (None, )
        xi_xj = tf.reduce_sum(tf.abs(xi_xj), -1)
        # xi_xj (None, )
        xi_xi = tf.reduce_sum(tf.pow(x_mat, 2), -1)
        # loss_mat_variance (None, )
        loss_mat_variance = xi_xj - xi_xi
    ...
    def _loss_norm_mean(self, materials_featurized):
        """
        sum_i sum_{j \neq i} (|x_i^T x_j|)
        differenciate mat from a random sampled material to increase the variance
        :param mat_pairs_featurized: (None, mat_feature_len)
        ...
        # x_mat_mean (latent_dim)
        x_mat_mean = tf.reduce_mean(x_mat, axis=0)

        # loss_mat_variance (None,)
        loss_mat_variance = tf.repeat(
            tf.norm(x_mat_mean),
            repeats=tf.shape(materials_featurized)[0]
        )
...
class PrecursorsPredict(keras.Model):
    def __init__(self,
        ...
        self.balance_PN = balance_PN
        self.precursor_frequency = tf.constant(np.array(
            self.precursor_counts[self.num_reserved_ids:]
        ).astype(np.float32)/self.num_train_reactions)
    ...
    def call(self, targets, precursors_conditional_indices=None, return_probability=True):
        """
        ...
        if precursors_conditional_indices is not None:
            ...
            precursors_conditional_mask = tf.greater_equal(precursors_conditional_indices, 0)
            ...
            precursors_conditional_mask = tf.cast(precursors_conditional_mask, tf.float32)
            # precursors_conditional_emb: (None, max_mats_num-1, latent_dim)
            precursors_conditional_emb = tf.gather(
                tf.transpose(precursor_kernel),
                precursors_conditional_indices
            )
            ...
            incomplete_reaction_emb = tf.concat(
                (
                    tf.expand_dims(targets_emb, axis=1),
                    precursors_conditional_emb
                ),
                axis=1
            )
    ...
    def _loss_cross_entropy(self,
        ...
        if self.predict_precursor_under_mask and precursors_conditional is not None:
            ...
            precursors_conditional_indices = tf.cast(
                precursors_conditional_labels - self.num_reserved_ids,
                tf.int32
            )
            # y_pred: (None, num_precursors)
        ...
        if self.balance_PN:
            loss = y_real*(-tf.math.log(y_pred)) + \
    ...
    def _loss_cross_entropy_sig_arctanh(self, reactions, reactions_featurized):
        ...
        # loss: (None, num_precursors, )
        if self.balance_PN:
            loss = y_real*(-tf.math.log(prob)) + \
                   (1-y_real)*(-tf.math.log(1-prob))*self.balance_coeff
        else:
            loss = y_real*(-tf.math.log(prob)) + \
                   (1-y_real)*(-tf.math.log(1-prob))
        # loss: (None, )
        loss = tf.reduce_sum(loss, axis=-1)
        return loss
    ...
    def _loss_regularization_1(self, reactions, reactions_featurized):
        ...
        precursors_indices = tf.cast(
            precursors_labels - self.num_reserved_ids,
            tf.int32
        )
        ...
        precursors_vectors = precursors_vectors * tf.expand_dims(precursors_mask, axis=2)
        ...
        dot_labels = tf.tile(
            tf.expand_dims(y_real, axis=1),
            multiples=[1, self.max_mats_num-1, 1]
        )
        ...
        regularization_term = regularization_term * tf.expand_dims(precursors_mask, axis=2)
    ...
    def _loss_cross_entropy_regularized_1(self, reactions, reactions_featurized):
        ...
        regularization_term_1 = self._loss_regularization_1(reactions, reactions_featurized)
    ...
    def predict_precursors(self, targets, precursors_conditional=None, output_thresh=0.5):
        """
...
class SynthesisTypePredict(keras.Model):
    def __init__(self,
                 num_eles,
                 max_mats_num,
                 latent_dim,
        ...
        self.latent_dim = latent_dim
        self.mat_encoder = mat_encoder
        # syn_type_labels is a tf lookup of {syn_type_str: syn_type_label}
        # label is the index in syn_type_counts
        # the real index starts from num_reserved_ids
        # the first num_reserved_ids indices are for
        # reserved symbols such as <MASK> <UNK>
        # syn_type_labels is int64
        self.syn_type_labels = syn_type_labels
        # mat_compositions is tf list of synthesis type string, index is syn_type_label
        self.syn_type_strings = syn_type_strings
        # mat_counts is python list of int, index is syn_type_label
        self.syn_type_counts = syn_type_counts
        self.num_train_reactions = num_train_reactions
        self.batch_size = batch_size

        self.vocab_size = len(syn_type_counts)
        self.num_reserved_ids = num_reserved_ids
        self.num_syn_types = self.vocab_size - self.num_reserved_ids
        ...
        self.balance_PN = balance_PN
        self.syn_type_frequency = tf.constant(np.array(
            self.syn_type_counts[self.num_reserved_ids:]
        ).astype(np.float32)/self.num_train_reactions)
        self.balance_coeff = self.syn_type_frequency/(1-self.syn_type_frequency)
        self.clip_logits = clip_logits

        if self.constrain_syn_type_layer:
            self.syn_type_layer = keras.layers.Dense(
                self.num_syn_types,
                activation=None,
                kernel_initializer=self.kernel_initializer,
                use_bias=self.bias_in_syn_type_layer,
                name='syn_type_layer',
                kernel_constraint=keras.constraints.UnitNorm(axis=0),
            )
        else:
    ...
    def call(self, targets, return_probability=True):
        """

        :param targets: (None, num_eles)
        :param return_probability: bool
        :return y_pred: (None, num_syn_types)
        """
        # targets_emb: (None, latent_dim)
        targets_emb, _, _ = self.mat_encoder(targets)

        # # Attention: this is redundant, just used to build syn_type_layer first
        # #   otherwise, tf would say self.syn_type_layer has no attribute kernel
        # # Change to torch in the future!
        # if not self.has_been_built:
        #     self.syn_type_layer(targets_emb)
        #     self.has_been_built = True

        # reactions_emb: (None, latent_dim)
        reactions_emb = targets_emb

        # y_pred: (None, num_syn_types)
        y_pred = self.syn_type_layer(reactions_emb)

        # print('dir(self.syn_type_layer)', dir(self.syn_type_layer))

        if self.norm_in_syn_type_projection:
            # self.syn_type_layer.kernel: (latent_dim, num_syn_types, )
            # kernel_norm: (num_syn_types, )
            kernel_norm = tf.sqrt(
                tf.reduce_sum(
                    tf.square(tf.transpose(self.syn_type_layer.kernel)),
                    axis=-1,
                )
            ) + 1e-12
            # y_pred: (None, num_syn_types)
            y_pred = y_pred/kernel_norm
            # the activation function in syn_type layer is None if norm_in_syn_type_projection == True
        y_pred = y_pred*self.dot_prod_scale
    ...
    def _loss_cross_entropy(self,
        ...
        :param syn_types: (None, )
        :param reactions_featurized: (None, max_mats_num, num_eles)
        :return loss: (None, )
        """
        # targets: (None, num_eles)
        targets = reactions_featurized[:, 0, :]

        # y_pred: (None, num_syn_types)
        y_pred = self.__call__(targets)

        # syn_types_labels: (None, )
        syn_types_labels = self.syn_type_labels.lookup(syn_types)
        # syn_types_labels: (None, 1)
        syn_types_labels = tf.expand_dims(syn_types_labels, axis=1)
        # y_real: (None, vocab_size, )
        y_real = tf.map_fn(
            lambda x: tf.scatter_nd(
                indices=tf.expand_dims(
        ...
        # y_real: (None, num_syn_types, )
        y_real = y_real[:, self.num_reserved_ids:]
        y_real = tf.cast(y_real, tf.float32)

        # TODO: do we want to use softmax?
        # loss: (None, num_syn_types, )
        if self.balance_PN:
            loss = y_real*(-tf.math.log(y_pred)) + \
               (1-y_real)*(-tf.math.log(1-y_pred))*self.balance_coeff
        else:
    ...
    def _loss_cross_entropy_sig_arctanh(self, syn_types, reactions_featurized):
        """
        equivalent to sigmoid(arctanh(dot(x_l,x_r)))
        ...
        :return loss: (None, )
        """
        assert self.norm_in_syn_type_projection == True
        # y_pred: (None, num_syn_types)
        targets = reactions_featurized[:, 0, :]
        y_pred = self.__call__(targets, return_probability=False)
        # y_pred is the logits here
    ...
    def _loss_cross_entropy_linear(self, syn_types, reactions_featurized):
        """
        equivalent to sigmoid(2*arctanh(dot(x_l,x_r)))
    ...
    def _loss_error_rate_unary(self, syn_types, reactions_featurized):
        """
        ...
        y_real = y_real[:, self.num_reserved_ids:]
        y_real = tf.cast(y_real, tf.float32)
        # y_correctness: (None, num_syn_types, )
        # y_correctness = (2.0*y_real-1.0)*y_pred + (1.0-y_real)
        y_error = tf.pow(y_pred-y_real, 2)
    ...
    def _loss_log_error_rate_unary(self, syn_types, reactions_featurized):
        error_rate = self._loss_error_rate_unary(syn_types, reactions_featurized)
        loss = tf.math.log(error_rate)
        return loss


    def _loss_error_rate(self, syn_types, reactions_featurized):
        """
    ...
    def _loss_log_error_rate(self, syn_types, reactions_featurized):
        error_rate = self._loss_error_rate(syn_types, reactions_featurized)
        loss = tf.math.log(error_rate)
        return loss

    def predict_synthesis_types(self, targets, output_thresh=0.5):
        """

        :param targets: (None, num_eles)
        :return all_pre_lists: list of list of (syn_type_str, score)
        """
        all_pre_lists = []
        ...
        for a_y in y_pred:
            pre_list_pred = self.syn_type_strings[self.num_reserved_ids:][
                a_y>output_thresh
            ].numpy()
            pre_score_pred = a_y[a_y>output_thresh].numpy()
            all_pre_lists.append([
                {
                    'syn_type': syn_type,
                    'score': score,
                } for (syn_type, score) in zip(pre_list_pred, pre_score_pred)
            ])
        return all_pre_lists


class ReactionCompare(keras.Model):
    def __init__(self,
                 num_eles,
                 max_mats_num,
                 mat_encoder,
    ...
    def call(self, inputs):
        ...
        """
        # inputs: (None, 2, max_mats_num, num_eles)
        # reaction_1: (None, max_mats_num, num_eles)
        (reaction_1, reaction_2) = tf.unstack(inputs, axis=1)
        target_1 = reaction_1[:, 0, :]
        precursors_1 = reaction_1[:, 1:, :]
        target_2 = reaction_1[:, 0, :]
        precursors_2 = reaction_1[:, 1:, :]
        # get mask (None, max_mats_num-1)
        pre_mask_1 = get_mat_mask_in_mat_seq(precursors_1, dtype=tf.float32)
        pre_mask_2 = get_mat_mask_in_mat_seq(precursors_2, dtype=tf.float32)

        # tar_emb_1 (None, latent_dim)
        tar_emb_1, _, _ = self.mat_encoder(target_1)
        tar_emb_2, _, _ = self.mat_encoder(target_2)

        all_pres_1 = tf.reshape(precursors_1, (-1, self.num_eles))
        pre_emb_1, _, _ = self.mat_encoder(all_pres_1)
        # pre_emb_1 (None, max_mats_num-1, latent_dim)
        pre_emb_1 = tf.reshape(
            pre_emb_1,
            (-1, self.max_mats_num-1, self.latent_dim)
        )
        pre_emb_1 = pre_emb_1 * tf.expand_dims(pre_mask_1, 2)
        # pre_emb_avg (None, latent_dim)
        pre_emb_avg_1 = tf.reduce_sum(
            pre_emb_1, axis=1
        )/tf.reduce_sum(
            pre_mask_1, axis=1, keepdims=True
        )

        all_pres_2 = tf.reshape(precursors_2, (-1, self.num_eles))
        pre_emb_2, _, _ = self.mat_encoder(all_pres_2)
        # pre_emb_2 (None, max_mats_num-1, latent_dim)
        pre_emb_2 = tf.reshape(
            pre_emb_2,
            (-1, self.max_mats_num-1, self.latent_dim)
        )
        pre_emb_2 = pre_emb_2 * tf.expand_dims(pre_mask_2, 2)
        # pre_emb_avg (None, latent_dim)
        pre_emb_avg_2 = tf.reduce_sum(
            pre_emb_2, axis=1
        )/tf.reduce_sum(
            pre_mask_2, axis=1, keepdims=True
        )

        # sim (None, )
        sim_tar = tf.reduce_sum(tar_emb_1*tar_emb_2, axis=-1)
        sim_pre = tf.reduce_sum(
            pre_emb_avg_1*pre_emb_avg_2,
            axis=-1
        )
        return tf.square(sim_tar - self.alpha * sim_pre)
