...
from SynthesisSimilarity.core import ML_utils
...
#         super().__init__(**kwargs)
#         self.all_elements = all_elements
#         self.target_compositions = target_compositions
#         self.output_thresh = output_thresh
#         self.featurizer_type = featurizer_type
#
#     def on_epoch_end(self, epoch, logs=None):
...
#         print()
#
#     def predict_precursors(self, model, target_compositions, to_print=True):
#         time_1 = datetime.datetime.now()
#
#         assert isinstance(model, MultiTasksOnRecipes)
...
#         assert len(target_compositions) > 0
#
#         if isinstance(target_compositions[0], dict):
#             comps = []
#             for tar in target_compositions:
#                 comps.append(
#                     composition_to_array(tar, self.all_elements)
#                 )
#             comps = featurize_list_of_composition(
#                 comps=comps,
#                 ele_order=self.all_elements,
...
#             comps, output_thresh=self.output_thresh
#         )
#
#         if to_print:
#             for (tar_comp, pre_list) in zip(comps, pre_lists_pred):
#                 pre_str_list = [
#                     (
#                         array_to_formula(comp['composition'], self.all_elements),
#                         comp['score']
#                     )
#                     for comp in pre_list
#                 ]
...
#         return pre_lists_pred


class PredictPrecursorsCallback_2(tf.keras.callbacks.Callback):
    def __init__(
        self,
        all_elements,
        mat_feature_len,
...
# class PredictSynthesisTypeCallback(tf.keras.callbacks.Callback):
#     def __init__(self,
#                  all_elements,
#                  test_data=None,
#                  output_thresh=-1.0,
...
#         )
#
#         # all_reactions_featurized: (batch_size, 2, max_mats_num, num_eles)
#         all_reactions_featurized = tf.stack(
#             [
#                 input_data['reaction_1_featurized'],
#                 input_data['reaction_2_featurized'],
...
#         all_synthesis_types = tf.stack(
#             [
#                 input_data['synthesis_type_1'],
#                 input_data['synthesis_type_2'],
#             ],
#             axis=1,
#         )
#         # all_reactions: (batch_size*2, )
#         all_synthesis_types = tf.reshape(
#             all_synthesis_types,
#             (-1, )
#         )
#         # TODO: need to print all real precursors too if available
#         self.target_compositions = all_reactions[:, 0, :]
#         self.target_compositions_featurized = all_reactions_featurized[:, 0, :]
#         self.all_synthesis_types = all_synthesis_types.numpy()
#
#     def on_epoch_end(self, epoch, logs=None):
#         print('the {}th epoch ends'.format(epoch))
#         print('logs', logs, logs.get('loss'))
#         print('Synthesis type prediction with targets only')
#         self.predict_synthesis_types(
#             model=self.model,
#             target_compositions=self.target_compositions,
#             target_features=self.target_compositions_featurized,
#             real_synthesis_types=self.all_synthesis_types,
#             to_print=True,
#         )
#         print()
#
#     def predict_synthesis_types(
#         self,
#         model,
#         target_compositions,
#         target_features=None,
#         real_synthesis_types=None,
#         to_print=True
#     ):
#         time_1 = datetime.datetime.now()
#
#         assert isinstance(model, MultiTasksOnRecipes)
#         assert 'syn_type' in model.task_names
#         assert len(target_compositions) > 0
#         if real_synthesis_types is not None:
#             assert len(target_compositions) == len(real_synthesis_types)
#
#         result_counter = collections.Counter()
#         pred_synthesis_types = []
#
#         if target_features is None:
#             target_features = featurize_list_of_composition(
#                 comps=target_compositions,
...
#         assert len(target_compositions) == len(target_features)
#
#         syn_type_lists_pred = model._model_by_task['syn_type'].predict_synthesis_types(
...
#
#                 if real_synthesis_types is not None:
#                     real_syn_type = real_synthesis_types[i]
#                 else:
#                     real_syn_type = None
#
#                 pre_str_list = [
...
#                         item['score']
#                     )
#                     for item in syn_type_lists_pred[i]
#                 ]
#                 # pre_str_list = syn_type_lists_pred[i]
#                 pre_str_list = sorted(pre_str_list, key=lambda x: x[1], reverse=True)
#                 pred_synthesis_types.append(pre_str_list[0][0])
#
#                 print('target: ', array_to_formula(tar_comp, self.all_elements))
#                 print('real synthesis type: ', str(real_syn_type))
#                 print('predicted synthesis types: ', str(pre_str_list))
#                 print()
#
#         if real_synthesis_types is not None:
#             result = ML_utils.evaluation_prediction(
#                 real_synthesis_types, pred_synthesis_types
#             )
#             pprint(result)
#
#         time_2 = datetime.datetime.now()
#         if to_print:
#             print('time cost in PredictSynthesisTypes: ', time_2 - time_1)
#         return syn_type_lists_pred


class PredictSimTarCallback(tf.keras.callbacks.Callback):
    # TODO: to be implemented
    # this is supposed to be an explicit prediction task
    def __init__(self, composition_pairs, featurizer_type="default", **kwargs):
        super().__init__(**kwargs)
        self.composition_pairs = composition_pairs
        self.featurizer_type = featurizer_type
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for pair in self.composition_pairs:
            ...
            comps = []
            for x in pair:
                comps.append(composition_to_array(x, self.model.all_eles))
            comps = featurize_list_of_composition(
                comps=comps,
                ele_order=self.model.all_eles,
                featurizer_type=self.featurizer_type,
            )
            v = self.get_mat_vector(comps)
            print(
                "Similarity between {} and {} \n= %.3f".format(
                    comp_1, comp_2, 1.0 - scipy.spatial.distance.consine(v[0], v[1])
                )
            )
        time_2 = datetime.datetime.now()
        print("time cost in SimTest: ", time_2 - time_1)
