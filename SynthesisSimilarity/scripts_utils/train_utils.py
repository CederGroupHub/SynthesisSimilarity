...
def get_composition_dict(struct_list, elements_vars={}):
    ...
    for tmp_struct in struct_list:
        if set(tmp_struct["elements"].keys()) == {"H", "O"} and len(struct_list) > 1:
...
def to_GeneralMat_obj(
    ...
    composition = get_composition_dict(raw_composition, elements_vars=elements_vars)
    ...
    try:
        ...
        edge_points = mat_obj.get_critical_compositions(skip_wrong_composition=True)
...
def load_valid_reactions(data_file):
    ...
    for i, reaction in enumerate(reactions):
        if i % 5000 == 0:
            print("{}/{} reactions loaded".format(i, len(reactions)))
        valid = True
        # print(reaction)
...
def composition_to_frac_formula(
    ...
    all_elements_indices = {ele: i for (i, ele) in enumerate(all_elements)}
...
def add_prototype_info(
    ...
    mat_hierarchy = make_hierarchy(
        materials=zip(base_materials, all_targets), name="AllMaterials"
    )
...
def load_synthesis_data(
    ...
    if not reload:
        ...
        effective_eles = sorted(effective_eles, key=lambda x: (Element(x).X, x))
        ...
        for r in reactions:
            ...
            r["target_comp"] = [
                utils.composition_to_array(comp.composition, effective_eles)
                for comp in tar
            ]
            for comp in r["target_comp"]:
                if np.sum(comp) == 0:
                ...
                if expand_reaction:
                    ...
                    for tar_pres_index in itertools.product(
                        *(
                            ...
                            + [
                                range(len(pre_comps))
                                for pre_comps in r["precursors_comp"]
                            ]
                        )
                    ):
                        ...
                        for i, pre_index in enumerate(tar_pres_index[1:]):
                            ...
                            if len(pre_eles) <= 2 and pre_eles.issubset(
                                {"O", "H", "N"}
                            ):
                                continue
...
def truncate_valence_array_in_reactions(
    ...
    # remove rare ions
    effective_ions = list(
        filter(lambda ion: ion_counter.get(ion, 0) >= ion_freq_threshold, all_ions)
    )
    ...
    if common_ion_not_feature:
        ...
        # remove common ions
        effective_ions = list(
            filter(lambda ion: ion not in common_ions, effective_ions)
        )

    # get indices of effective ions
    mapping_indices = [all_ions.index(ion) for ion in effective_ions]
    # rnx_to_remove = set()
    for k, r in enumerate(reactions):
        ...
        for i in range(len(r["precursors_valence"])):
            for j in range(len(r["precursors_valence"][i])):
                r["precursors_valence"][i][j] = r["precursors_valence"][i][j][
                    mapping_indices
                ]
...
def load_mp_data(data_file, ele_order, use_icsd_only=True):
    ...
    if use_icsd_only:
        ...
        materials = list(filter(lambda x: len(x["icsd_ids"]) > 0, materials))
    ...
    for i, mat in enumerate(materials):
        tmp_reaction = {
            "target_comp": [
                utils.composition_to_array(mat["reduced_cell_formula"], ele_order)
            ],
            "precursors_comp": [],
            "raw_index": i,
            "operations": [],
...
def load_common_precursors(
    all_elements, file_path="rsc/most_common_precursor.json", as_set=True
):
    ...
    for ele, x in list(common_precursors.items()):
        unified_formula = utils.array_to_formula(
            utils.formula_to_array(x, all_elements), all_elements
        )
        ...
        for p_p in precursor_frequencies[ele]:
            ...
            try:
                p_p["formula"] = utils.array_to_formula(
                    utils.formula_to_array(p_p["formula"], all_elements), all_elements
                )
            except:
                pass
...
def use_only_uncommon_reactions(
    ...
    common_precursors_set = set(
        [precursor_frequencies[ele][0]["formula"] for ele in precursor_frequencies]
    )
...
def prepare_dataset(
    ...
    for tmp_index, (r1, r2) in enumerate(
        itertools.combinations(effective_reactions, 2)
    ):
        ...
        r1_precursors_index = [
            random_gen.randrange(len(comps)) for comps in r1["precursors_comp"]
        ]
        r1_precursors = [
            r1["precursors_comp"][i][j] for i, j in enumerate(r1_precursors_index)
        ]
        ...
        r2_precursors_index = [
            random_gen.randrange(len(comps)) for comps in r2["precursors_comp"]
        ]
        r2_precursors = [
            r2["precursors_comp"][i][j] for i, j in enumerate(r2_precursors_index)
        ]
        ...
        data_dicts.append(
            {
                ...
                "reaction_1_featurized": [r1_target_featurized]
                + r1_precursors_featurized,
                "reaction_2_featurized": [r2_target_featurized]
                + r2_precursors_featurized,
...
def train_data_generator(
    ...
    def sample_data(all_reactions, weights=None):
        ...
        r2_target_featurized = r2["target_comp_featurized"][r2_target_index]
        r1_precursors_index = [
            random.randrange(len(comps)) for comps in r1["precursors_comp"]
        ]
        r1_precursors = [
            r1["precursors_comp"][i][j] for i, j in enumerate(r1_precursors_index)
        ]
        ...
        r2_precursors_index = [
            random.randrange(len(comps)) for comps in r2["precursors_comp"]
        ]
        r2_precursors = [
            r2["precursors_comp"][i][j] for i, j in enumerate(r2_precursors_index)
        ]
    ...
    data_batch_x = dataset_x.padded_batch(batch_size, padded_shapes=padded_data_shape)
