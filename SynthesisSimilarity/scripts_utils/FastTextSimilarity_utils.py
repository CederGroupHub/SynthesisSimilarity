...
def get_binary_composition_of_material_in_raw_reactions(
    ...
    binary_comp = elements_to_binary_composition(ele_in_mat, all_eles)
    return binary_comp
        ...
        for comp in r["target"]["composition"]:
            ...
            for (e1, e2) in itertools.combinations(comp["elements"].keys(), 2):
                ...
                if ele_pos[e1] <= ele_pos[e2]:
                    ...
                    left_ele = e2_subbed
                    right_ele = e1_subbed
                ele_order_counter["{} before {}".format(left_ele, right_ele)] += 1
...
def sort_elements_by_stat_order(elements, stat_ele_order):
    return sorted(
        elements,
        key=cmp_to_key(
            lambda ele_a, ele_b: compare_elements_by_stat_order(
                ele_a, ele_b, stat_ele_order
            )
        ),
    )
...
def compare_elements_by_stat_order(ele_a, ele_b, stat_ele_order):
    a_before_b = stat_ele_order.get("{} before {}".format(ele_a, ele_b), 0)
    b_before_a = stat_ele_order.get("{} before {}".format(ele_b, ele_a), 0)
    return b_before_a - a_before_b


def composition_to_human_formula(
    composition, raw_reaction, all_elements, stat_ele_order
):
    human_formula = None
    ...
    for ele in sorted_comp_eles:
        ...
        if composition[ele_index] == 1.0:
            ...
            human_formula += (
                "{}{:.3f}".format(ele, composition[ele_index]).rstrip("0").rstrip(".")
            )
